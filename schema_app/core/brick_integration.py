...
Interface between schema_app and brick_app
"""
...
try:
    from brick_app.core.brick_core_simple import BrickCore, SHACLBrick
    from brick_app.core.ontology_manager import OntologyManager
    from common import shared_library_manager
except Exception as e:
    print(f"Warning: Could not import brick_app modules: {e}")
    BrickCore = None
    SHACLBrick = None
...
class BrickIntegration:
    """Integration layer for working with brick_app"""
    
    def __init__(self, brick_repository_path: str = None, use_shared_libraries: bool = True):
        if BrickCore is None:
            raise ImportError("brick_app is not available. Please ensure it's properly installed.")
        
        # Always derive path from shared_library_manager — single source of truth
    ...
    def get_available_bricks(self, library_name: Optional[str] = None) -> List[SHACLBrick]:
        """Get all available bricks from brick_app"""
    ...
    def add_component_to_schema(self, schema_id: str, brick_id: str) -> bool:
        ...
        # This is a simplified implementation for the schema_app
