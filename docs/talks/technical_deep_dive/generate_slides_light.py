...
Output: docs/talks/technical_deep_dive/technical_deep_dive_slides_light.pptx
"""
...
C_ORANGE  = RGBColor(0xB8, 0x4A, 0x00)   # dark orange (readable on white)
...
def add_para(tf, text, font_size=16, bold=False, color=C_LIGHT, align=PP_ALIGN.LEFT):
    p = tf.add_paragraph()
    p.alignment = align
    r = p.add_run()
    r.text = text
...
def section_pill(slide, text, left=Inches(0.4), top=Inches(0.25)):
    """Now renders as a full-width title bar instead of a small pill."""
...
def heading(slide, text, font_size=34):
    add_textbox(slide, text, Inches(0.4), Inches(0.65), Inches(12.5), Inches(0.9),
...
def add_code_box(slide, code, left, top, width, height, font_size=13, border_color=C_ACCENT):
    ...
    for line in code.split("\n"):
        ...
        r.text = line
        r.font.size = Pt(font_size)
        r.font.color.rgb = RGBColor(0x02, 0x4A, 0x7A)   # dark blue on light grey
...
def slide_enrichment_layers12(prs):
    ...
    left.fill.solid(); left.fill.fore_color.rgb = RGBColor(0xE8,0xF4,0xFF)
    ...
    right.fill.solid(); right.fill.fore_color.rgb = RGBColor(0xED,0xFF,0xF3)
...
def main():
    ...
    out = Path(__file__).parent / "technical_deep_dive_slides_light.pptx"
    prs.save(str(out))
